...
// loop from 0 to just before the mirror point
  ...
  //Method to morph images between starting images and ending images
  //Changes current picture to morph betwen starting and ending picture
  public void morphStage (Picture startPicture, Picture endPicture, int numStages, int k){
    ...
    for (int x = 0; x <startPicture.getWidth(); x++){
      ...
      for (int y = 0; y<startPicture.getHeight();y++){
        //Sets pixelObj to refer to starting picture
        Pixel pixelObj = startPicture.getPixel(x,y);
        //Gets RGB values from beginning picture pixel
        int startRedValue = pixelObj.getRed();
        int startGreenValue = pixelObj.getGreen();
        int startBlueValue = pixelObj.getBlue();
        //Sets pixelObj1 to refer to ending picture
        Pixel pixelObj1 = endPicture.getPixel(x,y);
        //Sets pixelObj2 to refer to object image's pixel
        Pixel pixelObj2 = this.getPixel(x,y);
        //Gets RGB values from ending picture pixel
        int endRedValue = pixelObj1.getRed();
        int endGreenValue = pixelObj1.getGreen();
        int endBlueValue = pixelObj1.getBlue();
        //Calculates morphed picture RGB values based on morphing formula
        int redValue = startRedValue + ((endRedValue-startRedValue)/(numStages+1))*k;
        int greenValue = startGreenValue + ((endGreenValue-startGreenValue)/(numStages+1))*k;
        int blueValue = startBlueValue + ((endBlueValue-startBlueValue)/(numStages+1))*k;
        //Sets RGB values for object picture
        pixelObj2.setRed(redValue);
        pixelObj2.setGreen(greenValue);
        pixelObj2.setBlue(blueValue);
      }
    }
  }
}