public class Region {
	...
	public void add(QuadNode q) {
		...
		this.meanRed = (this.area * this.meanRed + quadNodeArea
				* q.getMeanRed())
				/ newArea;
		this.meanGreen = (this.area * this.meanGreen + quadNodeArea
				* q.getMeanGreen())
				/ newArea;
		this.meanBlue = (this.area * this.meanBlue + quadNodeArea
				* q.getMeanBlue())
				/ newArea;

		// Defn of variance: sigma^2=\frac{\sum_{i=1}^n (x_i-\bar{x})^2}{n}
		// This can be shown to be equal to: \frac{(\sum_{i=1}^n x_i^2 -
		// \bar{x}^2)}{n}
		// Therefore the \sum_{i=1}^n x_i^2 can be computed from
		// n(\sigma^2+\bar{x}^2)
		// for the region and the new quadnode. This allows a new standard
		// deviation (sigma)
		// to be computed for the new region after the quadnode is added to it.

		sum2Red = this.area
				* (this.sigmaRed * this.sigmaRed + this.meanRed * this.meanRed)
				+ quadNodeArea
				* (q.getSigmaRed() * q.getSigmaRed() + q.getMeanRed()
						* q.getMeanRed());
		sum2Green = this.area
				* (this.sigmaGreen * this.sigmaGreen + this.meanGreen
						* this.meanGreen)
				+ quadNodeArea
				* (q.getSigmaGreen() * q.getSigmaGreen() + q.getMeanGreen()
						* q.getMeanGreen());
		sum2Blue = this.area
				* (this.sigmaBlue * this.sigmaBlue + this.meanBlue
						* this.meanBlue)
				+ quadNodeArea
				* (q.getSigmaBlue() * q.getSigmaBlue() + q.getMeanBlue()
						* q.getMeanBlue());
		...
		// the quadnode to compute the new red, green and blue sigma values for
		// the region

		this.area = this.area + quadNodeArea;

		sigmaRed = Math.sqrt((sum2Red / this.area) - this.meanRed
				* this.meanRed);
		sigmaGreen = Math.sqrt((sum2Green / this.area) - this.meanGreen
				* this.meanGreen);
		sigmaBlue = Math.sqrt((sum2Blue / this.area) - this.meanBlue
				* this.meanBlue);
	}
	...
	public boolean adjacent(Region r, Boolean PRINT) {
		...
		for (int i = 0; i < this.quadqueue.size(); i++) {
			...
			if (PRINT) {
				...
				System.out.println("(" + q1X1 + "," + q1Y1 + ") " + "(" + q1X2
						+ "," + q1Y2 + ") " + "(" + q1X3 + "," + q1Y3 + ") "
						+ "(" + q1X4 + "," + q1Y4 + ") ");
			}
			...
			for (int j = 0; j < r.quadqueue.size(); j++) {
				...
				if (result == false
						&& (q1X1 + q1Side == q2X1)
						&& ((q1Y1 <= q2Y1 && q1Y1 >= q1Y2) || (q2Y1 <= q1Y1 && q2Y1 >= q2Y2)))
					result = true;
				if (result && PRINT)
					System.out.println("Adjaceny Condition 1 is true");

				// Right adjacency
				if (result == false
						&& (q2X1 + q2Side == q1X1)
						&& ((q2Y1 <= q1Y1 && q2Y1 >= q2Y2) || (q1Y1 <= q2Y1 && q1Y1 >= q1Y2)))
					result = true;
				if (result && PRINT)
					System.out.println("Adjaceny Condition 2 is true");

				// Top adjacency
				if (result == false
						&& (q1Y1 + q1Side == q2Y1)
						&& ((q1X1 <= q2X1 && q1X1 >= q1X2) || (q2X1 <= q1X1 && q2X1 >= q2X2)))
					result = true;
				if (result && PRINT)
					System.out.println("Adjaceny Condition 3 is true");

				// Bottom adjacency
				if (result == false
						&& (q2Y1 + q2Side == q1Y1)
						&& ((q2X1 <= q1X1 && q2X1 >= q2X2) || (q1X1 <= q2X1 && q1X1 >= q1X2)))
					result = true;
				if (result && PRINT)
					System.out.println("Adjaceny Condition 4 is true");

				if (!result && PRINT)
					System.out.println("Regions not adjacent" + "\n");
				if (result)
					return true;
			}
		}
	...
	public void mergeRegion(Region r, Boolean PRINT) {
		...
		for (int i = 0; i < r.quadqueue.size(); i++) {
			...
			this.quadqueue.enqueue(q); // size of queue will be updated
										// automatically
		}
		if (PRINT) {
			System.out.println("After loop: r.quadqueue.size()="
					+ r.quadqueue.size());
			System.out.println("After loop: this.quadqueue.size()="
					+ this.quadqueue.size());
		...
		if (PRINT)
			;
		{
			System.out.println("this.area=" + this.area + " regionArea="
					+ regionArea);
		...
		if (PRINT) {
			System.out.println("sum2Red term1:"
					+ this.area
					* (this.sigmaRed * this.sigmaRed + this.meanRed
							* this.meanRed));
			System.out.println("sum2Red term2:"
					+ regionArea
					* (r.getSigmaRed() * r.getSigmaRed() + r.getMeanRed()
							* r.getMeanRed()));
		...
		sum2Green = this.area
				* (this.sigmaGreen * this.sigmaGreen + this.meanGreen
						* this.meanGreen)
				+ regionArea
				* (r.getSigmaGreen() * r.getSigmaGreen() + r.getMeanGreen()
						* r.getMeanGreen());
		sum2Blue = this.area
				* (this.sigmaBlue * this.sigmaBlue + this.meanBlue
						* this.meanBlue)
				+ regionArea
				* (r.getSigmaBlue() * r.getSigmaBlue() + r.getMeanBlue()
						* r.getMeanBlue());
		if (PRINT) {
		...
		this.meanRed = (this.area * this.meanRed + regionArea * r.getMeanRed())
				/ newRegionArea;
		this.meanGreen = (this.area * this.meanGreen + regionArea
				* r.getMeanGreen())
				/ newRegionArea;
		this.meanBlue = (this.area * this.meanBlue + regionArea
				* r.getMeanBlue())
				/ newRegionArea;
		...
		this.sigmaRed = Math.sqrt((sum2Red / this.area) - this.meanRed
				* this.meanRed);
		this.sigmaGreen = Math.sqrt((sum2Green / this.area) - this.meanGreen
				* this.meanGreen);
		this.sigmaBlue = Math.sqrt((sum2Blue / this.area) - this.meanBlue
				* this.meanBlue);
		if (PRINT) {
			System.out.println("this.sigmaRed=" + this.sigmaRed);
			System.out.println("sum2Red=" + sum2Red);
			System.out.println("this.meanRed^2=" + this.meanRed * this.meanRed);
			System.out.println("sqrt term:"
					+ (sum2Red - this.meanRed * this.meanRed));
			System.out.println("this.area=" + this.area);
